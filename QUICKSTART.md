...
Any user can clone this scaffold, drop their existing resume into the `cv/` folder, and have the AI agent take complete hold of their data, build their profile, tailor 1-page LaTeX CVs and cover letters, and track applications.
...
```bash
git clone <your-repo-url> ai-job-search
cd ai-job-search
```
...
## 📋 Prerequisites
- **Python 3.10+**
- **Bun** (for job search CLIs): `curl -fsSL https://bun.sh/install | bash`
- **LaTeX** (for compiling CVs and cover letters):
  - macOS: MacTeX or BasicTeX
  - Linux: `sudo apt install texlive-luatex texlive-xetex texlive-latex-extra`
  - Windows: MiKTeX
